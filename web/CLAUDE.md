...
- Frames (D-062): the root layout draws none, so each screen family renders its own `<main id="contenu">`, the skip link's target: `components/shared/app-shell.tsx` for every signed-in space (the role's screens in `SPACES`, first item the dashboard; a new screen adds its item there), the home page, the `(auth)` layout, `/capture/[token]`, `not-found.tsx` and `error.tsx`. `inverse-*` is the dark inverted ground of the navigation column and the home page's closing call; a section on it re-scopes the focus ring with `[--ring:var(--inverse-foreground)]`. The logo is `public/logo.jpg`, referenced once as `LOGO_SRC` in `components/shared/logo.tsx`; its extension must match its real format, and the Dockerfile copies `public/`.
...
| 2026-09-13 | team | Frames and `main` per screen family, navigation column, dashboards, page guides, brochure home page, logo placeholder (D-062) |
| 2026-09-13 | team | Real logo at `public/logo.jpg`, extension matching its format |
