...
/**
 * The frame of every signed-in space (docs/design.md section 4): a navy navigation column with the
...
// Ghost buttons default to the light ground's hover colours; these keep them legible on the navy one.
...
export function AppShell({ user, children }: { user: User; children: ReactNode }): JSX.Element {
  ...
  return (
    <div className="min-h-dvh lg:grid lg:grid-cols-[16rem_minmax(0,1fr)]">
      {/* The ring token is re-scoped so keyboard focus stays visible on the navy ground. */}
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    current
                      ? "bg-inverse-accent font-medium text-inverse-foreground before:absolute before:inset-y-1.5 before:left-0 before:w-1 before:rounded-full before:bg-brand"
