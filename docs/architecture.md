...
| `capture_link` | id, user_id, organisation_id, token_sha256 (unique), document_id (nullable), created_at, expires_at | A link a laptop shows as a QR code so a phone files one document for that user and organisation without signing in; `document_id` is set when it is used, which ends it (G3, D-056) |
...
- `POST /capture/links?organisation_id=` [msme, accountant] - a phone capture link for one of the user's organisations, `{id, token, expires_at, document_id}`; the token is returned only here, and the link is valid 10 minutes and for one document (D-056)
...
| 2026-09-13 | team | Phone capture: several `file` parts filed as one PDF (D-055); `capture_link` (section 4) and the `/capture/*` endpoints (section 5), per D-056 |
