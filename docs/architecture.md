...
- `GET /impact/activity` [same as `GET /impact`] - for the dashboards: files by status, files filed per day over the last 14 days in Tunisian time (zero-filled), files with a schema-valid export, and the 8 latest files (F1, D-062)
...
| 2026-09-13 | team | API surface: `GET /impact/activity` for the dashboards (D-062) |
