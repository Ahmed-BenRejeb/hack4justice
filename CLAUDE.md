...
The outcome is decided by a 3-minute pitch and a live demo. Everything in this repo serves one of the demo moments listed in docs/plan.md section 12, or it does not get built.
...
| Path | What it is |
|---|---|
| `app/` | Routes. `(msme)`, `(officer)`, `(admin)` route groups, plus `api/` handlers |
| `components/` | React components: `ui/` primitives, `msme/`, `officer/`, `shared/` |
| `lib/` | Domain logic: `extraction/`, `rules/`, `corpus/`, `counterparty/`, `export/`, `db/`, `auth/`, and `env.ts` |
| `rules/` | The rule registry: rule definitions with their verbatim legal citations |
...
| `docs/` | plan, design, facts register, decision log |
...
- Phases have gates (docs/plan.md section 14). Do not start a phase until the previous gate is met. When a gate is at risk, cut from the cut list, do not slip the gate.
...
- **No fact on a slide or on screen that is not in docs/facts.md with status `verified`.** Article numbers are checked by a person against the official source, never recalled from memory and never taken from a model.
- Configuration enters the process in exactly one module: `lib/env.ts`. It is the only place that reads the environment.
  ...
  - Algorithm parameters (confidence thresholds, benefit calculation constants) are not identity. They may keep a documented default in `lib/config.ts`.
  ...
  - `.env` and `.env.example` carry the identical key set in the same order. Copying `.env.example` to `.env` must produce a working run.
...
- The model provider is swappable. Never import a vendor SDK outside `lib/extraction/provider.ts`, and never name a vendor model anywhere else.
...
- Commit messages follow Conventional Commits: `type(scope): summary`
  ...
  - Scope: the directory or module touched (app, components, lib, rules, corpus, export, seed, docs). Omit scope only for repo-wide changes.
