...
this registry (docs/decision-log.md D-030). It fires at upload, on the
...
def decide_matricule_fiscal_format(facts: dict[str, str]) -> RuleOutcome:
    ...
    could not read one.
    """
    supplier_tax_id = facts.get("supplier_tax_id", "").strip()
    if not supplier_tax_id:
        return Abstention(missing_fact="supplier_tax_id")

    if MATRICULE_PATTERN.match(supplier_tax_id.upper()):
        return Decision(code="TEJ_MATRICULE_VALID")
    return Decision(code="TEJ_MATRICULE_INVALID")
