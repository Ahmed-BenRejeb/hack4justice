...
- `app/counterparty/` does not exist. RNE's own site (`home.registre-entreprises.tn`) returned HTTP 503 on every path checked 2026-09-12, and its data-exchange API appears to require a formal "service contract" subscription rather than a self-serve key like OpenRouter's. See `docs/facts.md`. Revisit once the site is reachable and the real access process is known.
